/**
 * Slug → hero video assets for the 9 boats with shipped footage. Lives
 * outside the boat data source so it applies regardless of whether
 * boats come from the dummy file or Supabase.
 *
 * Each entry references a yoyo-encoded mp4 (forward + reversed in one
 * file) at two resolutions plus a first-frame poster used as the LCP
 * image and the <video poster> fallback. Encoded by
 * scripts/encode-yacht-videos.sh.
 */
